...
public class Collector
{
	...
	private Map<Integer,Objects> _objets = new TreeMap<Integer,Objects>();
	...
	//Les logs
	private Map<Integer,Objects> _LogObjets = new TreeMap<Integer,Objects>();
	...
	public ArrayList<Drop> getDrops() {
		...
		for(Objects obj : _objets.values()) {
	...
	public Map<Integer, Objects> getObjets() 
	{
		return _objets;
	}
		...
		for(Entry<Integer, Collector> perco : World.getPercos().entrySet())
		{
			 if(perco.getValue().get_guildID() == GuildID)
    		 {
	    			...
	    			if(perco.getValue().get_inFight() == 1)
	    			{
	    				...
	    				{
	    					packet.append(Config.CONFIG_MS_FOR_START_FIGHT).append(";");//TimerActuel
	    				}else
	    				{
	    					packet.append(perco.getValue().get_turnTimer()).append(";");//TimerActuel
	    				}
	    				packet.append(Config.CONFIG_MS_FOR_START_FIGHT).append(";");//TimerInit
	    				packet.append("7;");//Nombre de place maximum FIXME : En fonction de la map
	    				packet.append("?,?,");//?
	    			}else
	    			{
	    				packet.append("0;");
	    				packet.append(Config.CONFIG_MS_FOR_START_FIGHT).append(";");
	    				packet.append("7;");
	    				packet.append("?,?,");
	    			}
	...
	public static void parseAttaque(Characters perso, int guildID)
	{
		for(Entry<Integer, Collector> perco :  World.getPercos().entrySet()) 
		{
	...
	public static void parseDefense(Characters perso, int guildID)
	{
		for(Entry<Integer, Collector> perco :  World.getPercos().entrySet()) 
		{
			if(perco.getValue().get_inFight() > 0 && perco.getValue().get_guildID() == guildID)
	...
	public void removeFromPercepteur(Characters P, int guid, int qua)
	{
		Objects PercoObj = World.getObjet(guid);
		Objects PersoObj = P.getSimilarItem(PercoObj);
		...
		if(PersoObj == null)//Si le joueur n'avait aucun item similaire
		{
			...
			}else //S'il reste des objets
			{
				...
				PersoObj = Objects.getCloneObjet(PercoObj, qua);
	...
	public void LogObjetDrop(int guid, Objects obj)
	{
	...
	public String get_LogItems()
	{
		...
		for(Objects obj : _LogObjets.values())
	...
	public static void removePercepteur(int GuildID)
	{
		for(Entry<Integer, Collector> perco : World.getPercos().entrySet())
		{
			if(perco.getValue().get_guildID() == GuildID)
			{
				...
				for(Characters p : World.getCarte((short) perco.getValue().get_mapID()).getPersos())
				{
