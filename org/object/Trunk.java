...
public class Trunk {
	...
	private int _cellid;
	private Map<Integer, Objects> _object = new TreeMap<Integer, Objects>();
	private long _kamas;
	private String _key;
	...
	public Map<Integer, Objects> get_object()
	{
		return _object;
	}
	...
	public static void LockTrunk(Characters P, String packet) 
	{
	...
	public void HopIn(Characters P)//Ouvrir coffre
	{
		...
		Trunk t = P.getInTrunk();
		Houses h = World.getHouse(_house_id);
		
		if(t == null) return;
		if(t.get_owner_id() == P.getAccID() || (P.get_guild() == null ? false : P.get_guild().get_id() == h.get_guild_id() && h.canDo(Constant.C_GNOCODE)))
		{
			OpenTrunk(P, "-", true);
		}
	...
	public static void OpenTrunk(Characters P, String packet, boolean isTrunk)//Ouvrir un coffre
	{	
		Trunk t = P.getInTrunk();
		if(t == null) return;
	...
	public static void closeCode(Characters P)
	{
		SocketManager.GAME_SEND_KODE(P, "V");
	}
	
	public boolean isTrunk(Characters P, Trunk t)//Savoir si c'est son coffre
	{
		if(t.get_owner_id() == P.getAccID()) return true;
		else return false;
	}
	
    public static ArrayList<Trunk> getTrunksByHouse(Houses h)
    {
	...
	public String parseToTrunkPacket()
	{
		...
		for(Objects obj : _object.values())
	...
	public void addInTrunk(int guid, int qua, Characters P)
	{
		...
		{
			Logs.addToGameLog("Le joueur "+P.get_name()+" a tenter d'ajouter un objet dans un coffre qu'il n'avait pas.");
			return;
		}
		if (P.getItems().get(guid).getQuantity() < qua)
		{
			Logs.addToGameLog("Le joueur "+P.get_name()+" a tenté d'ajouter une quantité d'objet en banque dont il ne possédait pas.");
			return;
		}
		...
		Objects TrunkObj = getSimilarTrunkItem(PersoObj);
		...
		if(TrunkObj == null)//S'il n'y pas d'item du meme Template
		{
			...
			else//S'il reste des objets au joueur
			{
				...
				TrunkObj = Objects.getCloneObjet(PersoObj, qua);
		...
		for(Characters perso : P.get_curCarte().getPersos())
		{
	...
	public void removeFromTrunk(int guid, int qua, Characters P)
	{
		if(P.getInTrunk().get_id() != get_id()) return;
		
		Objects TrunkObj = World.getObjet(guid);
		if(TrunkObj == null) return;
		//Si le joueur n'a pas l'item dans son coffre
		if(_object.get(guid) == null)
		{
			Logs.addToGameLog("Le joueur "+P.get_name()+" a tenter de retirer un objet dans un coffre qu'il n'avait pas.");
			return;
		}
		// On ajoute une vérif pour éviter une déconnexion
		if(P.getItems().get(guid) != null){
			if (P.getItems().get(guid).getQuantity() < qua) {
				Logs.addToGameLog("Le joueur " + P.get_name() + " a tenté de retirer une quantité d'objet dans un coffre dont il ne possédait pas.");
				return;
			}
		}
		Objects PersoObj = P.getSimilarItem(TrunkObj);
		...
		if(PersoObj == null)//Si le joueur n'avait aucun item similaire
		{
			...
			}else //S'il reste des objets dans le coffre
			{
				...
				PersoObj = Objects.getCloneObjet(TrunkObj, qua);
	...
	private Objects getSimilarTrunkItem(Objects obj)
	{
		for(Objects value : _object.values())
		{
	...
	public String parseTrunkObjetsToDB()
	{
		...
		for(Entry<Integer,Objects> entry : _object.entrySet())
		{
	...
	{
		for(Entry<Integer, Objects> obj : get_object().entrySet())
		{
	...
	public void moveTrunktoBank(Accounts Cbank)
	{
		for(Entry<Integer, Objects> obj : get_object().entrySet())
		{
