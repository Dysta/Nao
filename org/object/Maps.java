...
public class Maps {
	...
	public static class MountPark {
		...
		public static void removeGuildMountParks(int GuildID) {
			try {
				for (Entry<Short, org.object.Maps.MountPark> mp : World.getMountPark().entrySet())// Pour chaque enclo si ils en ont plusieurs
				{
					if (mp.getValue().get_guild().get_id() == GuildID) {
						if (!mp.getValue().getData().isEmpty()) {
							for (Entry<Integer, Integer> MPdata : mp.getValue().getData().entrySet()) {
								...
								SQLManager.REMOVE_MOUNT(MPdata.getKey());// Suppression des dindes dans chaque denclo
							}
						}
						...
						for (Characters p : mp.getValue().get_map().getPersos()) {
		...
		public static void removeMapMountPark(int mapID){
	        try {
	            for (Entry<Short, org.object.Maps.MountPark> mp : World.getMountPark().entrySet())// Pour chaque enclos si ils en ont plusieurs
	            {
	                if (mp.getValue().get_guild() == null && mp.getValue().get_owner() > 0 && mp.getValue().get_map().get_id() == mapID){
	                    if (!mp.getValue().getData().isEmpty()) {
	                        for (Entry<Integer, Integer> MPdata : mp.getValue().getData().entrySet()) {
	                            World.removeDragodinde(MPdata.getKey());// Suppression des dindes dans le world
		...
		public static void removeAllMountPark(){
	        try {
	            for (Entry<Short, org.object.Maps.MountPark> mp : World.getMountPark().entrySet())// Pour chaque enclos si ils en ont plusieurs
	            {
	                if (mp.getValue().get_guild() == null && mp.getValue().get_owner() > 0){
	                    if (!mp.getValue().getData().isEmpty()) {
	                        for (Entry<Integer, Integer> MPdata : mp.getValue().getData().entrySet()) {
	                            World.removeDragodinde(MPdata.getKey());// Suppression des dindes dans le world
	                            SQLManager.REMOVE_MOUNT(MPdata.getKey());// Suppression des dindes dans chaque enclo
	...
	public static class Case {
		private int _id;
		private Map<Integer, Characters> _persos; // = new TreeMap<Integer,
													// Personnage>();
		...
		private Objects _droppedItem;
		private Timer _timer;
		private byte _state;
		private byte _tempMov;
		private byte _movement;
		private boolean _actived;

		public Case(Maps a_map, int id, boolean _walk, boolean LoS, int objID, boolean activo, byte movimiento) {
			_map = a_map.get_id();
			_mapOb = a_map;
			_id = id;
			_Walkable = _walk;
			_actived = activo;
			_movement = movimiento;
			_state = 1;
			_tempMov = this._movement;
			_LoS = LoS;
			if (objID == -1)
				return;
		...
		public synchronized void activeCell() {
			if (this._state != 1)
				return;
			this._movement = 4;
			SocketManager.GAME_SEND_GDC_PACKET(World.getCarte(this._map), this._id + ";aaVaaaaaaa800");
			SocketManager.GAME_SEND_GDF_PACKET(World.getCarte(this._map), this._id + ";" + 2);
			this._state = 3;
			this._timer = new Timer(30000, new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					Case.this._timer.stop();
					Case.this._movement = Case.this._tempMov;
					Case.this._state = 1;
					SocketManager.GAME_SEND_GDF_PACKET(World.getCarte(Case.this._map), Case.this._id + ";" + 4);
					try {
						Thread.sleep(1500L);
					} catch (Exception localException) {
					}
					SocketManager.GAME_SEND_GDC_PACKET(World.getCarte(Case.this._map), Case.this._id + ";aa"
							+ CryptManager.getHashedValueByInt(Case.this._movement << 3 & 0x38) + "aaaaaaa800");
				}
			});
			this._timer.start();
		}

		public synchronized void activeIncarnamCell() {
			if (this._state != 1)
				return;
			this._movement = 4;
			SocketManager.GAME_SEND_GDC_PACKET(World.getCarte(this._map), this._id + ";aaGaaaaaaa801");
			SocketManager.GAME_SEND_GDF_PACKET(World.getCarte(this._map), this._id + ";" + 2);
			this._state = 3;
			this._timer = new Timer(30000, new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					SocketManager.GAME_SEND_GDC_PACKET(World.getCarte(Case.this._map), Case.this._id + ";aa"
							+ CryptManager.getHashedValueByInt(Case.this._movement << 3 & 0x38) + "aaaaaaa801");
				}
			});
			this._timer.start();
		}
		...
		public boolean canDoAction(int id) {
			...
			default:
				Logs.addToGameLog("MapActionID non existant dans Case.canDoAction: " + id);
				return false;
			}
		}
		...
		public void applyOnCellStopActions(Characters perso) {
			if (_onCellStop == null)
				return;
		...
		public synchronized void addPerso(Characters perso) {
			if (_persos == null)
				_persos = new TreeMap<Integer, Characters>();
		...
		public boolean isWalkable(boolean useObject) {
			if ((!this._actived) || (this._movement == 0) || (this._movement == 1))
				return _Walkable;
		...
		public Map<Integer, Characters> getPersos() {
			if (_persos == null)
				return new TreeMap<Integer, Characters>();
			Map<Integer, Characters> p = new TreeMap<Integer, Characters>();
			p.putAll(_persos);
			return p;
		}
		...
		public void startAction(Characters perso, GameAction GA) {
			int actionID = -1;
			short CcellID = -1;
			try {
			...
			case 102:// Puiser
				...
				if (_object.getState() != Constant.IOBJECT_STATE_FULL)
					return;// Si le puits est vide
				_object.setState(Constant.IOBJECT_STATE_EMPTYING);
				_object.setInteractive(false);
				SocketManager.GAME_SEND_GA_PACKET_TO_MAP(perso.get_curCarte(), "" + GA._id, 501, perso.get_GUID() + "",
						_id + "," + _object.getUseDuration() + "," + _object.getUnknowValue());
			...
			case 157: // Zaapis
				...
				if (perso.get_curCarte()._subArea.getArea().getID() == 7
						&& (perso.get_align() == 1 || perso.get_align() == 0 || perso.get_align() == 3))// Ange,
																										// Neutre
																										// ou
																										// Sérianne
				{
					...
					if (perso.get_align() == 1)
						price = 10;
				} else if (perso.get_curCarte()._subArea.getArea().getID() == 11
						&& (perso.get_align() == 2 || perso.get_align() == 0 || perso.get_align() == 3))// Démons,
																										// Neutre
																										// ou
																										// Sérianne
				{
			...
			case 183:// Retourner sur Incarnam
				...
				short mapID = Constant.getStartMap(perso.get_classe());
				int cellID = Constant.getStartCell(perso.get_classe());
				perso.teleport(mapID, cellID);
				perso.get_compte().getGameThread().removeAction(GA);
				break;
			case 81:// Vérouiller maison
				Houses h = Houses.get_house_id_by_coord(perso.get_curCarte().get_id(), CcellID);
				if (h == null)
					return;
				perso.setInHouse(h);
				h.Lock(perso);
			...
			case 84:// Rentrer dans une maison
				Houses h2 = Houses.get_house_id_by_coord(perso.get_curCarte().get_id(), CcellID);
				if (h2 == null)
					return;
				perso.setInHouse(h2);
				h2.HopIn(perso);
			...
			case 97:// Acheter maison
				Houses h3 = Houses.get_house_id_by_coord(perso.get_curCarte().get_id(), CcellID);
				if (h3 == null)
					return;
				perso.setInHouse(h3);
				h3.BuyIt(perso);
			...
			case 104:// Ouvrir coffre privé
				...
				if (trunk == null) {
					Logs.addToGameLog(
							"Game: INVALID TRUNK ON MAP : " + perso.get_curCarte().get_id() + " CELLID : " + CcellID);
					return;
				}
			...
			case 105:// Vérouiller coffre
				...
				break;
			case 153: // Poubelle
				Trunk poub = Trunk.get_trunk_id_by_coord(perso.get_curCarte().get_id(), CcellID);
				if (poub == null) {
					Logs.addToGameLog(
							"Game: INVALID TRUNK ON MAP : " + perso.get_curCarte().get_id() + " CELLID : " + CcellID);
					return;
			...
			case 108:// Modifier prix de vente
				Houses h4 = Houses.get_house_id_by_coord(perso.get_curCarte().get_id(), CcellID);
				if (h4 == null)
					return;
				perso.setInHouse(h4);
				h4.SellIt(perso);
			...
			default:
				Logs.addToGameLog("Case.startAction non definie pour l'actionID = " + actionID);
				break;
			}
		}

		public void finishAction(Characters perso, GameAction GA) {
			int actionID = -1;
			try {
				actionID = Integer.parseInt(GA._args.split(";")[1]);
			} catch (Exception e) {
			...
			case 102:// Puiser
				...
				Objects obj = World.getObjTemplate(311).createNewItem(qua, false, -1);
			...
			default:
				Logs.addToGameLog("[FIXME]Case.finishAction non definie pour l'actionID = " + actionID);
				break;
			}
		}
		...
		public void addDroppedItem(Objects obj) {
			_droppedItem = obj;
		}
	...
	public Maps(short _id, String _date, byte _w, byte _h, String _key, String places, String dData, String cellsData,
			String monsters, String mapPos, byte maxGroup, byte maxSize) {
		this._id = _id;
		this._date = _date;
		this._w = _w;
		this._h = _h;
		...
		try {
			...
			if (_subArea != null)
				_subArea.addCarte(this);
		} catch (Exception e) {
			Logs.addToGameLog("Erreur de chargement de la map " + _id + ": Le champ MapPos est invalide");
			System.exit(0);
		}
		...
		if (!dData.isEmpty()) {
			...
			for (String o : cellsDataArray) {
				...
				_cases.put(Number, new Case(this, Number, Walkable, LineOfSight, obj, true, (byte) 7));
			}
		}
		...
		if (Config.CONFIG_USE_MOBS) {
			if (_maxGroup == 0)
				return;
			spawnGroup(Constant.ALIGNEMENT_NEUTRE, _maxGroup, false, -1);// Spawn
																			// des
																			// groupes
																			// d'alignement
																			// neutre
			spawnGroup(Constant.ALIGNEMENT_BONTARIEN, 1, false, -1);// Spawn du
																	// groupe de
																	// gardes
																	// bontarien
																	// s'il y a
			spawnGroup(Constant.ALIGNEMENT_BRAKMARIEN, 1, false, -1);// Spawn du
																		// groupe
																		// de
																		// gardes
																		// brakmarien
	...
	public void applyEndFightAction(int type, Characters perso) {
	...
	public NPC addNpc(int npcID, int cellID, int dir) {
		NpcTemplates temp = World.getNPCTemplate(npcID);
		if (temp == null)
			return null;
		if (getCase(cellID) == null)
			return null;
		NPC npc = new NPC(temp, get_nextObjectID(), cellID, (byte) dir);
		_npcs.put(get_nextObjectID(), npc);
		set_nextObjectID(get_nextObjectID() - 1);
		return npc;
	}

	public void spawnAfterTimeGroup(int i, int j, boolean b, int k) {
		int nbr = 1;
		if (respawnTimer != null)
			nbr++;
		final int nbrFinal = nbr;
		final int map = this.get_id();
		respawnTimer = new Timer(20000, new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				System.out.println("Repop.");
				respawnTimer.stop();
				World.getCarte((short) map).spawnGroup(-1, nbrFinal, true, -1);// Respawn
																				// d'un
																				// groupe
				respawnTimer.stop();
			}
		...
		respawnTimer.start();
		System.out.println("Respawn Timer start.");
	}

	public void spawnAfterTimeGroupFix(final int cell) {
		final int map = this.get_id();
		final java.util.Map<String, String> mobData = World.getGroupFix(map, cell);
		respawnTimer = new Timer(Integer.parseInt(mobData.get("timer")), new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				World.getCarte((short) map).addStaticGroup(cell, mobData.get("groupData"));
				respawnTimer.stop();
			}
		});
		respawnTimer.start();
	...
	public void spawnGroup(int align, int nbr, boolean log, int cellID) {
		if (nbr < 1)
			return;
		if (this._mobGroups.size() - this._fixMobGroups.size() >= this._maxGroup)
			return;
		for (int a = 1; a <= nbr; a++) {
			MobGroup group = new MobGroup(this._nextObjectID, align, this._mobPossibles, this, cellID, this._maxSize);
			if (group.getMobs().isEmpty())
				continue;
			this._mobGroups.put(this._nextObjectID, group);
			if (log) {
				Logs.addToGameLog("- Un groupe de monstre viens d'être ajouté sur la Map (" + this._id
						+ ") d'alignement " + align + " avec comme ID : " + this._nextObjectID + " !");
	...
	public void spawnNewGroup(boolean timer, int cellID, String groupData, String condition) {
		MobGroup group = new MobGroup(this._nextObjectID, cellID, groupData);
		if (group.getMobs().isEmpty())
			return;
		this._mobGroups.put(this._nextObjectID, group);
	...
	public void spawnGroupOnDead(int align, int nbr, boolean log, int cellID, MobGroup group) {
		if (nbr < 1)
			return;
		for (int a = 1; a <= nbr; a++) {
			if (!group.getMobs().isEmpty()) {
				_mobGroups.put(Integer.valueOf(group.getID()), group);
				if (log) {
					Logs.addToGameLog("Groupe de monstres ajoutes sur la map: " + this._id + " alignement: " + align
							+ " ID: " + this.get_nextObjectID());
					SocketManager.GAME_SEND_MAP_MOBS_GM_PACKET(this, group);
				}
				this.set_nextObjectID(this.get_nextObjectID() - 1);
			}
		}
	}
	...
	public void addStaticGroup(int cellID, String groupData) {
		MobGroup group = new MobGroup(get_nextObjectID(), cellID, groupData);
		if (group.getMobs().isEmpty())
			return;
		_mobGroups.put(get_nextObjectID(), group);
		set_nextObjectID(get_nextObjectID() - 1);
		_fixMobGroups.put(-1000 + get_nextObjectID(), group);
		SocketManager.GAME_SEND_MAP_MOBS_GM_PACKET(this, group);
	}
	...
	public ArrayList<Characters> getPersos() {
		ArrayList<Characters> persos = new ArrayList<Characters>();
		synchronized (_persos) {
			persos.addAll(_persos.values());
		}
		return persos;
		/*
		 * try { for(Case c : _cases.values()) for(Personnage entry :
		 * c.getPersos().values()) persos.add(entry); } catch(Exception e){
		 * e.printStackTrace(); Ancestra.addToDebug("Bug getPeros"); } return
		 * persos;
		 */
	}
	...
	public void addPlayer(Characters perso) {
		SocketManager.GAME_SEND_ADD_PLAYER_TO_MAP(this, perso);
		perso.get_curCell().addPerso(perso);
	}

	public void sendGMsPackets(Characters p_out) {
		/*
								 * StringBuilder packet = new StringBuilder();
								 * ArrayList<Case> cases = new
								 * ArrayList<Case>(); synchronized(_cases){
								 * cases.addAll(_cases.values()); } for(Case
								 * cell : cases)for(Personnage perso :
								 * cell.getPersos().values())packet.append(
								 * "GM|+").append(perso.parseToGM()).append('\
								 * u0000'); return packet.toString();
								 */
	...
	public String getMobGroupGMsPackets() {
		if ((this._mobGroups.isEmpty()) && (!this._fixMobGroups.isEmpty()))
			for (MobGroup MB : _fixMobGroups.values()) {
				addStaticGroup(MB.getCellID(), MB.getGroupData());
			}
	...
	public Fight newFight(Characters init1, Characters init2, int type, boolean targetProtected) {
		int id = 1;
		if (!_fights.isEmpty())
	...
	public Fight newKoli(ArrayList<Characters> team1, ArrayList<Characters> team2) {
		int id = 1;
		if (!_fights.isEmpty())
			id = ((Integer) (_fights.keySet().toArray()[_fights.size() - 1])) + 1;
	...
	public String getRandomFightPos() {
		...
		for (int i = 0; i < 8; i++) {
			int rand = Formulas.getRandomValue(0, freeCells.size() - 1);
			team1.add(freeCells.get(rand));
			freeCells.remove(rand);
		}
		for (int i = 0; i < 8; i++) {
			int rand = Formulas.getRandomValue(0, freeCells.size() - 1);
			team2.add(freeCells.get(rand));
			freeCells.remove(rand);
		}
		for (Integer cellt1 : team1) {
			pos.append(CryptManager.cellID_To_Code(cellt1));
		}
		pos.append("|");
		for (Integer cellt2 : team2) {
			pos.append(CryptManager.cellID_To_Code(cellt2));
		}
		return pos.toString();
	}
	...
	public int getRandomFreeCellID() {
		...
		if (freecell.isEmpty()) {
			Logs.addToGameLog("Aucune cellulle libre n'a ete trouve sur la map " + _id + " : groupe non spawn");
			return -1;
		}
		int rand = Formulas.getRandomValue(0, freecell.size() - 1);
		return freecell.get(rand);
		/*
		 * int max = _cases.size()-_w; int rand = 0; int lim = 0; boolean
		 * isOccuped;
		 * 
		 * do { isOccuped = false; rand = Formulas.getRandomValue(_w,max);
		 * if(lim >50) return 0; for(Entry<Integer,MobGroup> group :
		 * _mobGroups.entrySet()) { if (group.getValue().getCellID() != 0) {
		 * if(group.getValue().getCellID() ==
		 * _cases.get(_cases.keySet().toArray()[rand]).getID()) isOccuped =
		 * true; } } for(Entry<Integer,NPC> npc : _npcs.entrySet()) {
		 * if(npc.getValue().get_cellID() ==
		 * _cases.get(_cases.keySet().toArray()[rand]).getID()) isOccuped =
		 * true; }
		 * 
		 * if (_cases.get(_cases.keySet().toArray()[rand]).isWalkable() &&
		 * !isOccuped) { return
		 * _cases.get(_cases.keySet().toArray()[rand]).getID(); }
		 * 
		 * lim++;
		 * }while(!_cases.get(_cases.keySet().toArray()[rand]).isWalkable() &&
		 * !isOccuped);
		 * 
		 * return 0; //
		 */
	...
	public void refreshSpawns() {
		...
		spawnGroup(Constant.ALIGNEMENT_NEUTRE, _maxGroup, true, -1);// Spawn des
																	// groupes
																	// d'alignement
																	// neutre
		spawnGroup(Constant.ALIGNEMENT_BONTARIEN, 1, true, -1);// Spawn du
																// groupe de
																// gardes
																// bontarien
																// s'il y a
		spawnGroup(Constant.ALIGNEMENT_BRAKMARIEN, 1, true, -1);// Spawn du
																// groupe de
																// gardes
																// brakmarien
																// s'il y a
	...
	public void onPlayerArriveOnCell(Characters perso, int caseID) {
		if (_cases.get(caseID) == null)
			return;
		Objects obj = _cases.get(caseID).getDroppedItem();
		if (obj != null) {
			if (perso.addObjet(obj, true))
				World.addObjet(obj, true);
		...
		for (MobGroup group : _mobGroups.values()) {
			if (Pathfinding.getDistanceBetween(this, caseID, group.getCellID()) <= group.getAggroDistance())// S'il
																											// y
																											// aggro
			{
				if ((group.getAlignement() == -1 || ((perso.get_align() == 1 || perso.get_align() == 2)
						&& (perso.get_align() != group.getAlignement())))
						&& ConditionParser.validConditions(perso, group.getCondition())) {
					if (perso.get_compte().get_subscriber() == 0 && getSubArea().get_subscribe()
							&& Config.USE_SUBSCRIBE)
						return;
					Logs.addToGameLog(perso.get_name() + " lance un combat contre le groupe " + group.getID()
							+ " sur la map " + _id);
	...
	public void startFigthVersusMonstres(Characters perso, MobGroup group) // Vérifier
																			// à
																			// chaque
																			// combat
																			// l'ID
	...
	public void startFigthVersusPercepteur(Characters perso, Collector perco) {
		int id = 1;
		if (!_fights.isEmpty())
			id = ((Integer) (_fights.keySet().toArray()[_fights.size() - 1])) + 1;
	...
	public void startFightVSPrisme(Characters perso, Prism Prisme) {
		int id = 1;
		if (!_fights.isEmpty())
			id = ((Integer) (_fights.keySet().toArray()[_fights.size() - 1])) + 1;
	...
	public boolean canFight() {
		if (_placesStr.isEmpty() || _placesStr.equalsIgnoreCase("|")) {
			return false;
		}
		try {
			String[] str = _placesStr.split("\\|");
	...
	public Maps getMapCopy() {
		...
		String pos = _placesStr;
		if (!canFight()) {
			pos = getRandomFightPos();
		}

		Maps map = new Maps(_id, _date, _w, _h, _key, pos);
		...
		for (Entry<Integer, Case> entry : _cases.entrySet())
			cases.put(entry.getKey(),
					new Case(map, entry.getValue().getID(), entry.getValue().isWalkable(false),
							...
							entry.getValue()._actived, entry.getValue()._movement));
		map.setCases(cases);
		return map;
	}
	...
	public void sendFloorItems(Characters perso) {
		for (Case c : _cases.values()) {
	...
	public synchronized void addPerso(Characters P) {
		_persos.put(P.get_GUID(), P);
	}

	public synchronized void removePerso(int P) {
		if (_persos.containsKey(P))
			_persos.remove(P);
	}

	public int getRandomNearFreeCellID(int cellid)// obtenir une cell aléatoire
													// et proche
	{
		ArrayList<Integer> freecell = new ArrayList<Integer>();
		...
		if (freecell.isEmpty()) {
			Logs.addToGameLog("Aucune cellulle libre n'a ete trouve sur la map " + _id + " groupe non déplacé");
			return -1;
		}
		int rand = Formulas.getRandomValue(0, freecell.size() - 1);
		return freecell.get(rand);
	...
	public void onMap_MonstersDisplacement() {
		...
		for (Entry<Integer, MobGroup> entry : getMobGroups().entrySet()) {
			...
			for (Characters z : getPersos()) {
				SocketManager.GAME_SEND_GA_PACKET(z.get_compte().getGameThread().get_out(), "0", "1",
						entry.getValue().getID() + "", pathstr);
			}
		}
	...
	public void set_nextObjectID(int _nextObjectID) {
		this._nextObjectID = _nextObjectID;
	}
}