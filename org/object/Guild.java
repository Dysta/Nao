...
public class Guild {
	...
	public Guild(Characters owner,String name,String emblem)
	{
			...
			String sorts, String stats)
	{
		...
		statsFight.clear();
		statsFight.put(Constant.STATS_ADD_FORC, _lvl);
		statsFight.put(Constant.STATS_ADD_SAGE, get_Stats(Constant.STATS_ADD_SAGE));
		statsFight.put(Constant.STATS_ADD_INTE, _lvl);
		statsFight.put(Constant.STATS_ADD_CHAN, _lvl);
		statsFight.put(Constant.STATS_ADD_AGIL, _lvl);
		statsFight.put(Constant.STATS_ADD_RP_NEU, (int)Math.floor(get_lvl()/2));
		statsFight.put(Constant.STATS_ADD_RP_FEU, (int)Math.floor(get_lvl()/2));
		statsFight.put(Constant.STATS_ADD_RP_EAU, (int)Math.floor(get_lvl()/2));
		statsFight.put(Constant.STATS_ADD_RP_AIR, (int)Math.floor(get_lvl()/2));
		statsFight.put(Constant.STATS_ADD_RP_TER, (int)Math.floor(get_lvl()/2));
		statsFight.put(Constant.STATS_ADD_AFLEE, (int)Math.floor(get_lvl()/2));
		statsFight.put(Constant.STATS_ADD_MFLEE, (int)Math.floor(get_lvl()/2));
	}
	...
	public GuildMember addNewMember(Characters p)
	{
	...
	public void removeMember(Characters perso)
	{
	...
	public void levelUp()
	{
		_lvl++;
		_capital += 5;
		// On met l'embleme dorée
		if (_lvl >= 200) {
			String[] data = _emblem.split(",");
			int bgid = Integer.parseInt(Integer.valueOf(data[0], 36).toString());
			bgid+= 17;
			_emblem = Integer.toString(bgid, 36) + "," + data[1] + "," + data[2] + "," + data[3];
		}
	}
	...
	public String parsePercotoGuild()
	{
		...
		packet.append(Collector.CountPercoGuild(get_id())).append("|");
