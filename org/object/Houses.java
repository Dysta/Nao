...
public class Houses
{
	...
	public static void LoadHouse(Characters P, int newMapID)//Affichage des maison + Blason
	{
		...
		for(Entry<Integer, Houses> house : World.getHouses().entrySet())
		{
			if(house.getValue().get_map_id() == newMapID)
			{
				...
				{
					Accounts C = World.getCompte(house.getValue().get_owner_id());
	...
	public void HopIn(Characters P)//Entrer dans la maison
	{
	...
	public static void OpenHouse(Characters P, String packet, boolean isHome)//Ouvrir une maison ;o
	{
	...
	public void BuyIt(Characters P)//Acheter une maison
	{
		Houses h = P.getInHouse();
	...
	public static void HouseAchat(Characters P)//Acheter une maison
	{
		Houses h = P.getInHouse();
		...
		if(h.get_owner_id() > 0)
		{
			Accounts Seller = World.getCompte(h.get_owner_id());
		...
		for(Characters z:P.get_curCarte().getPersos())
		{
			LoadHouse(z, z.get_curCarte().get_id());
		}
	}
	
	public void SellIt(Characters P)//Vendre une maison
	{
		Houses h = P.getInHouse();
		if(isHouse(P, h))
		{
	...
	public static void SellPrice(Characters P, String packet)//Vendre une maison
	{
		Houses h = P.getInHouse();
	...
	public boolean isHouse(Characters P, Houses h)//Savoir si c'est sa maison
	{
	...
	public static void closeBuy(Characters P)
	{
		SocketManager.GAME_SEND_hOUSE(P, "V");
	}
	...
	public static void LockHouse(Characters P, String packet) 
	{
		Houses h = P.getInHouse();
		if(h.isHouse(P, h))
		{
	...
	public static String parseHouseToGuild(Characters P)
	{
	...
	public static boolean AlreadyHaveHouse(Characters P)
	{
		for(Entry<Integer, Houses> house : World.getHouses().entrySet())
		{
			if(house.getValue().get_owner_id() == P.getAccID())
	...
	public static void parseHG(Characters P, String packet)
	{
		Houses h = P.getInHouse();
	...
	public static void Leave(Characters P, String packet)
	{
		Houses h = P.getInHouse();
		if(!h.isHouse(P, h)) return;
		int Pguid = Integer.parseInt(packet);
		Characters Target = World.getPersonnage(Pguid);
	...
	public static Houses get_HouseByPerso(Characters P)//Connaitre la MAPID + CELLID de sa maison
	{
		for(Entry<Integer, Houses> house : World.getHouses().entrySet())
		{
			if(house.getValue().get_owner_id() == P.getAccID())
